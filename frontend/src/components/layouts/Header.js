...
import {Navbar, Nav, Container, Toast, NavDropdown, Badge} from "react-bootstrap";
...
//SERVICES
import { useGetProfileMutation } from "redux/slices/profileApiSlice";
...
function Header() {
    ...
    useEffect(() => {
        ...
        const getLoggedUser = async (token) => {
            try{
                //Con el unwrap(), obtengo defrente la respuesta y si hay algun error el error sera el catch "err" y es lo mismo de arriba
    ...
    return (    
        <header style={{position: "relative"}}>
            ...
            <Toast show={showToastError} onClose={() => {setShowToastError(false)}} position={"top-center"} bg={"danger"} delay={3000} autohide className="m-3" style={{position: "absolute", zIndex: 1, bottom: 0, left: "50%", transform: "translate(-50%, 100%)"}}>
                <Toast.Body className={"text-white"}>{message}</Toast.Body>
            </Toast>

            {/* Para que me diga que estoy logueado */}
            <Toast show={showToastIsLogged} onClose={() => {setShowToastIsLogged(false)}} position={"top-center"} bg={"success"} delay={3000} autohide className="m-3" style={{position: "absolute", zIndex: 1, bottom: 0, left: "50%", transform: "translate(-50%, 100%)"}}>
                <Toast.Body className={"text-white"}>Bienvenido {userInfo.name}!</Toast.Body>
            </Toast>
                ...
                <Container>
                    ...
                    >
                        ...
                        <Nav.Link onClick={() => navigate("/cart")}>
                            ...
                            {qtyItems > 0 && <Badge pill bg='success' style={{marginLeft: '.5rem'}}>{qtyItems}</Badge>}                        
                        </Nav.Link>
                        {
                            isLogged
                            ?
                            <NavDropdown title={isLoading ? "Cargando..." : `${userInfo.name}`} id="username">
                            ...
                            :
                            <Nav.Link onClick={() => navigate("/auth/login")}><i className="fas fa-user"></i> Sign In</Nav.Link>
                        }
