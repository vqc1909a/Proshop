...
import { useEditUserMutation, useGetUserByIdQuery} from "redux/slices/usersApiSlice"
...
function ModalEditUser(props) {
  ...
  const schema = yup.object().shape({
    ...
    password: yup.string().min(6, "La cantidad mínima de carácteres es 6").required("El password es requerido")
  });

  const [editUser, {isError, isSuccess ,isLoading, error, data, reset}] = useEditUserMutation();
  
  const {data: dataUser, isError: isErrorUser, isLoading: isLoadingUser, error: errorUser} = useGetUserByIdQuery({token, userId: userIdSelected});
  ...
  if(errorUser?.status === 401 || errorUser?.status === 403){
    dispatch(ERROR_ACTIONS.saveMessage(errorUser?.data?.message || errorUser?.error))
  ...
  return (
    ...
    >
      ...
      {/* Si no pones nada de estas condicionales no pasa nada pero lo hacemos mas que nada al obtener el producto y puede dar error */}
      {
        ...
        isErrorUser
        ?
        <Message variant="danger">{errorUser?.data?.message || errorUser?.error}</Message>
        :
        <Modal.Body>
          ...
          <Formik
            ...
            onSubmit={async (values) => {
              const token = localStorage.getItem("token") ? JSON.parse(localStorage.getItem("token")) : "";
              try{
                await editUser({token, userId: values.id, newUser: values}).unwrap()  
              }catch(err){
