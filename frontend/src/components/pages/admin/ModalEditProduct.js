...
import { useEditProductMutation, useGetProductByIdQuery,   } from "redux/slices/productsApiSlice"
...
function ModalEditProduct(props) {
  ...
  const schema = yup.object().shape({
    ...
    price: yup.number("El precio es un número").positive("El precio es un número positivo").required("El precio es requerido"),
    countInStock: yup.number("El stock es un número").integer("El stock debe ser un número entero").min(0, 'El stock debe ser igual o mayor que 0').required("El stock es requerido")
  });

  const [editProduct, {isError, isSuccess ,isLoading, error, data, reset}] = useEditProductMutation();
  
  const {data: dataProduct, isError: isErrorProduct, isLoading: isLoadingProduct, error: errorProduct} = useGetProductByIdQuery({token, idProduct: idProductSelected});
  ...
  if(errorProduct?.status === 401 || errorProduct?.status === 403){
    dispatch(ERROR_ACTIONS.saveMessage(errorProduct?.data?.message || errorProduct?.error))
  ...
  return (
    ...
    >
      ...
      {/* Si no pones nada de estas condicionales no pasa nada pero lo hacemos mas que nada al obtener el producto y puede dar error */}
      {
        ...
        isErrorProduct
        ?
        <Message variant="danger">{errorProduct?.data?.message || errorProduct?.error}</Message>
        :
        <Modal.Body>
          ...
          <Formik
            ...
            onSubmit={async (values) => {
                const token = localStorage.getItem("token") ? JSON.parse(localStorage.getItem("token")) : "";
                try{
                  await editProduct({token, idProduct: values.id, newProduct: values}).unwrap()  
                }catch(err){
