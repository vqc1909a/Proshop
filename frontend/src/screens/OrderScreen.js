...
function OrderScreen() {
    ...
    const { isError: isErrorGetOrder, isLoading: isLoadingGetOrder, data: dataGetOrder, error: errorGetOrder, /* refetch */} = useGetOrderByIdQuery({token, orderId});
    ...
    const {data: payPalClientId, isError: isErrorClientId ,error: errorClientId } = useGetPayPalClientIdQuery();
    const clientId = payPalClientId?.clientId ;
    const [deliverOrder, {isError: isErrorDeliver, isSuccess: isSuccessDeliver, isLoading: isLoadingDeliver, data: dataDeliver, error: errorDeliver}] = useDeliverOrderMutation();

    //Mutations
    const [payOrder, {isLoading: isLoadingPayOrder, isError: isErrorPayOrder, isSuccess: isSuccessPayOrder, data: dataPayed, error: errorPayOrder }] = usePayOrderMutation();
    ...
    const [{ isPending, /* isInitial, isRejected, isResolved */ }, paypalDispatch] = usePayPalScriptReducer();
    ...
    return isLoadingGetOrder ? (
        ...
        <>
            ...
            <Message variant="danger">{errorGetOrder?.data?.message || errorGetOrder?.error}</Message>
        </>
    ) : (
        <>
            ...
            <Row>
                        ...
                        <ListGroup.Item>
                            ...
                            {order.items.map(({productId, qty}, index) => (
                                <ListGroup.Item key={index}>
                                    <Row>
                                        <Col md={1}>
                                            <Image src={productId?.image } alt={productId?.name } fluid rounded></Image>
                                        </Col>
                                        <Col>
                                        ...
                                        <Col md={4}>
                                            {qty} * ${productId?.priceIVA || 0} = ${qty * productId.priceIVA}
                                        </Col>
                                    </Row>
                                </ListGroup.Item>
                            ))}
                ...
                <Col md={4}>
                    <Card>
                        {isErrorDeliver && <Message variant="danger">{errorDeliver?.data?.message || errorDeliver?.error}</Message>}
                        {isErrorClientId && <Message variant="danger">{errorClientId?.data?.message || errorClientId?.error}</Message>}
                        {isErrorPayOrder && <Message variant="danger">{errorPayOrder?.data?.message || errorPayOrder?.error}</Message>}
                        {isSuccessPayOrder && <Message variant="success">{dataPayed?.message }</Message>}
                        {isSuccessDeliver && <Message variant="success">{dataDeliver?.message }</Message>}
                                            ...
                                            <div>
                                                ...
                                                */}
                                                    ...
                                                    //Ocurre cuando en la UI de Paypal le das al boton de pagar luego de elegir tu metodo de pago en el mismo paypal (VISA O SALDO DEL MIMSO PAYLPAL)
                                                        ...
                                                        onCancel={() => console.log("Cancelamos la transaccion")}
                                                        onShippingChange={() => console.log("Cambiando dirección de envío")}
                                                        // className = ""
                                                        // style={{}}
                                                        // disabled
                                                        // forceReRender=[]
