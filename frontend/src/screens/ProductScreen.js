...
function ProductScreen(){
    ...
    const { data, isError, isLoading, error, refetch} = useGetProductBySlugQuery(slug);
	...
	const handleAddToCart = (product, qty) => {
		...
		let {userId, description, createdAt, updatedAt, rating, numReviews, ...restProduct} = product;
		let newProduct = {
			...restProduct, 
			qty
		}	
	...
	return (
		<>
			{isLoading 
				...
				(
				<>
					<Meta title={selectedProduct.name} description={selectedProduct.description} keywords={`${selectedProduct.name}, ${selectedProduct.brand}, ${selectedProduct.category}`}></Meta>
                        ...
                        <Col md={3}>
                            <Card>
                                <ListGroup variant="flush">
                                    ...
                                    <ListGroup.Item>
                                        <Button onClick={() => handleAddToCart(selectedProduct, qty)} className="btn btn-block w-100" disabled={countInStock === 0} type="button">
                                            Add To Cart
                                        </Button>
                                    </ListGroup.Item>
                                </ListGroup>
					...
					<Reviews selectedProduct={selectedProduct} refetchProduct={refetch}></Reviews>
				</>
				)
			}
		</>
