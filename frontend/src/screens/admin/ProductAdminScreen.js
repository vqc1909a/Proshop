...
import { useDeleteProductMutation, useGetMyProductsQuery } from "redux/slices/productsApiSlice"
...
function ProductAdminScreen() {
  ...
  const { isError, isLoading, data, error, /* refetch */ } = useGetMyProductsQuery({token, page});
  ...
  const handleDeleteProduct = (idProduct) => {
    Swal.fire({
      title: 'Estás seguro de eliminar este producto y los comentarios recibidos?',
  ...
  return (
    <>
            ...
            <tbody>
              {
                products.map((product) => (
                    <tr key={product.id}>
                      <td><Link to={`/products/${product.slug}`}>{product.id}</Link></td>
                      <td>{product.name}</td>
                      <td>
                      ...
                      <td>
                        <div className="d-flex justify-content-between">
                          <TooltipButton title="Editar Producto" id="t-1">
                            <Button variant="info" className="btn-sm" onClick={() =>  handleEditProduct(product.id)}><i className="fas fa-pencil"></i></Button>
                          </TooltipButton>
                          <TooltipButton title="Editar Imagen" id="t-2">
                            <Button variant="success" className="btn-sm" onClick={() => handleEditImageProduct(product.id)}><i className="fas fa-image"></i></Button>
                          </TooltipButton>
                          <TooltipButton title="Eliminar Producto" id="t-2">
                            <Button variant="danger" className="btn-sm" onClick={() => handleDeleteProduct(product.id)}><i className="fas fa-trash"></i></Button>
                          </TooltipButton>
                        </div>
                      </td>
                    </tr>
      ...
      {
        products.length > 0 && (
          <Paginate pathname="/admin/products" totalPages={totalPages} page={pageNow}></Paginate>
        )
      }
