...
function UserAdminScreen() {
  ...
  const { isError, isLoading, data, error, /* refetch */ } = useGetUsersQuery({token, page});
  ...
  return (
    <>
            ...
            <tbody>
              {
                users.map((user) => (
                    <tr key={user.id}>
                      ...
                      <td><span style={{display: "flex", justifyContent: "center", alignItems: "center"}}><i className={`fas fa-${user.isAdmin ? 'check' : 'times'}`} style={{color: `${user.isAdmin ? 'green' : 'red'}`}}></i></span></td>
                      <td>
                        <div className="d-flex justify-content-around">
                          <TooltipButton title="Editar Usuario" id="t-1">
                            <Button variant="info" className="btn-sm" onClick={() =>  handleEditUser(user.id)}><i className="fas fa-pencil"></i></Button>
                          </TooltipButton>
                          <TooltipButton title="Eliminar Usuario" id="t-2">
                            <Button variant="danger" className="btn-sm" onClick={() => handleDeleteUser(user.id)}><i className="fas fa-trash"></i></Button>
                          </TooltipButton>
                        </div>
                      </td>
                    </tr>
      ...
      {
        users.length > 0 && (<Paginate totalPages={totalPages} page={pageNow} pathname="/admin/users"></Paginate>)
      }
