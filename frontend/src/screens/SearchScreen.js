...
import { useGetProductBySearchQuery } from 'redux/slices/productsApiSlice';
...
function SearchScreen() {
  ...
  const { data, isError, isLoading, error} = useGetProductBySearchQuery({keyword, page});
  ...
  return (
    <>
        ...
        {
          products.length > 0 && (
            <Paginate pathname={`/search/${keyword}`} totalPages={totalPages} page={pageNow}></Paginate>
          )
        }
    </>
  )
