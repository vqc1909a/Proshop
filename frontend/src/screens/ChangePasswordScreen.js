...
function ChangePasswordScreen() {
  ...
  const [updatePassword, { isError, isSuccess, isLoading, data, error}] =  useUpdatePasswordMutation();

  const {current_password, new_password, confirm_new_password, form: passwords, setForm, handleChange} = useForm(PASSWORD_DEFAULT);
  
  const submitUpdatePassword = async ({event, passwords, errorRequest, isSuccessRequest}) => {
  ...
  return (
    <>
        ...
        <FormContainer>
            ...
            <Form onSubmit={(event) => submitUpdatePassword({event, passwords, errorRequest: error, isSuccessRequest: isSuccess})}>
                <Form.Group controlId="current_password" className="mb-3">
                    <Form.Label>Current Password</Form.Label>
                    <Form.Control type="password" name="current_password" placeholder="Enter current password" value={current_password} onChange={handleChange}></Form.Control>
                </Form.Group>

                <Form.Group controlId="new_password" className="mb-3">
                    <Form.Label>New Password</Form.Label>
                    <Form.Control type="password" name="new_password" placeholder="Enter new password" value={new_password} onChange={handleChange}></Form.Control>
                </Form.Group>

                <Form.Group controlId="confirm_new_password" className="mb-3">
                    <Form.Label>Confirm New Password</Form.Label>
                    <Form.Control type="password" name="confirm_new_password" placeholder="Confirm new password" value={confirm_new_password} onChange={handleChange}></Form.Control>
                </Form.Group>
                {
                    isLoading 
                    ?
                ...
                <Button variant="link" className="ms-3" onClick={() => handleButtonGoBack()}>Go Back</Button> 
            </Form>
        </FormContainer>
    </>
  )
